...
{
    d_echo(">>>>>>>>>> - MIB processed ".$mib."\n");
    $data = snmp_walk($device, $oid, $snmpflags, $mib, $mibdir);
    foreach (explode("\n", $data) as $entry) {
	d_echo(">>>>>>>>>>> Entry ".$entry."\n");
        list($oid,$value)  = explode('=', $entry, 2);
        $oid               = trim($oid);
	d_echo(">>>>>>>>>>> OID found: ".$oid."\n");
        $value             = trim($value, "\" \\\n\r");
	d_echo(">>>>>>>>>>> OID Value found : ".$value."\n");
        list($oid, $index) = explode('.', $oid, 2);
	if($mib == "TOSHIBA-ES-MIB" && !isset($index)) // supports TOSHIBA-ES-MIB
	    $index=0; 
        d_echo(">>>>>>>>>>> Index value : ".$index."\n");
	if (!strstr($value, 'at this OID') && isset($oid) && isset($index)) {
	    if(!strstr($entry,'browser.')) // TOSHIBA-ES-MIB WRONG ENTRY
            	$array[$index][$oid] = $value;
	    else
		d_echo(">>>>>>>>>> Entry non corretta ".$entry."\n");
        }
	d_echo("\n");
	/*else
	{	if(!strstr($entry,'browser.'))
		{
			$array[0][$oid] = $value;
			d_echo(">>>>>>>>>> Value scartato ".$value."\n\n");
		}
		else
			d_echo(">>>>>>>>>> Value non corretto ".$value."\n\n");
	}*/
    }
    return $array;
...
function snmp_mib_translate($oid, $module, $mibdir = null, $device = array())
{
    ...
    //$cmd .= " -IR -m $module $oid";
    $cmd .= " -m $module $oid";
    //$cmd .= " -m $module .1.3.6.1.4.1.20008.1.1";
...
d_echo("******************* oids\n");
    ...
    foreach ($oids as $index => $array) {
	    ...
	    {	
            	d_echo("******************* obj ". $obj."\n");
            	d_echo("******************* oid string ". $mibdef[$obj]['oid']."\n");
            	d_echo("******************* oid value ". $val."\n");
...
function load_mibdefs($module, $name)
{
    ...
    //d_echo("####################### prefix: ".$prefix."\n");
    foreach ($result as $mib => $m) {
    //    d_echo("####################### mib row ". $m." prefix ". $prefix ." strlen: ".strlen($prefix)."\n");
	if (strlen($prefix) > 2) {
            $result[$mib]['shortname'] = preg_replace("/^$prefix/", '', $m['object_type'], 1);
		//d_echo("####################### 1 result for ".$mib." :" .$result[$mib]['shortname']."\n");
        } else {
            $result[$mib]['shortname'] = $m['object_type'];
	//d_echo("####################### 2 result for ".$mib." :" .$result[$mib]['shortname']."\n");
        }
    }
...
function register_mibs($device, $mibs, $included_by)
{
    ...
    d_echo(Config::getOsSetting($device['os'], 'register_mibs', array()));
    d_echo($mibs);
    foreach ($mibs as $name => $module) {
	d_echo(">>>>>>>>>>>>> Name ".$name."\n");
	d_echo(">>>>>>>>>>>>> Module ".$module."\n");
